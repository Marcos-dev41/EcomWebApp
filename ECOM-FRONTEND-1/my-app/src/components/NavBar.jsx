...
// fixed login state issue 
  ...
  return (
    <div className=' flex flex-row justify-around w-full bg-orange-500 h-fit p-1 items-center font-semibold shadow-md'>
        ...
        <input className='bg-orange-100 rounded-2xl w-80 p-2 pl-5' type="search" value={search} placeholder='search...' onChange={((e)=>{setSearch(e.target.value)})}/>
        ...
        <div className='flex flex-row justify-between pl-5 pr-5 gap-4'>
            ...
            <Link to={"/cart"}>
                {cart.length > 0 ?
                <div className='flex'>
                  <img src={addedcart} alt="" width=" 30px" height="30px" className='opacity-85 hover:scale-102'/> 
